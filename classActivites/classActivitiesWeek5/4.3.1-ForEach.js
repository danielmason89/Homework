...
let coordinates = [];
data.for((data) => {
            let latLng = `${data.address.geo.lat} + ${data.address.geo.lat};
...
let arrOfStrings = [];
data.forEach((data) => {
    let catchPhraseAndBs = 
    `
            catchPhrase: $ { data.company.catchPhrase }, BS: $ { data.company.bs }
            `
    arrOfStrings.push(catchPhraseAndBs);
});
